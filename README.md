...
* **error_threshold**. The amount of errors a worker encounters within error_window_timeout amount of time before
  opening the circuit, that is to start rejecting requests instantly.
* **error_window_timeout**. The duration of the rolling window in seconds. Defaults to error_timeout seconds if not set.
