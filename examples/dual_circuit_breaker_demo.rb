...
# simultaneously, switching between them at runtime based on a feature flag.
...
resource = Semian.register(
  ...
  # Provide an experiment flag proc that determines which circuit breaker to use
  # Returns true = use adaptive, false = use legacy
  experiment_flag_proc: -> { FeatureFlags.enabled?(:use_adaptive_circuit_breaker) },
...
# Test with legacy circuit breaker (flag disabled)
puts "Phase 1: Using LEGACY circuit breaker (flag=false)"
puts "-" * 50
...
# Test with adaptive circuit breaker (flag enabled)
puts "\nPhase 2: Using ADAPTIVE circuit breaker (flag=true)"
puts "-" * 50
...
puts "was used for decision-making based on the experiment flag."
