...
Semian::NetHTTP.semian_configuration = proc do |host, port|
  ...
  if host == "shopify.com"
    puts "  [semian/http] set resource name shopify_com with any port".gray
...
puts " >> 4. Request to http://shopify.com:81/index.html - fail".magenta
begin
  Net::HTTP.start("shopify.com", 81, open_timeout: 3) do |http|
    http.request_get(bad_host)
  end
...
puts " >> 5. Request to HELTHY http://shopify.com:/index.html - fail".magenta.bold
begin
  Net::HTTP.start("shopify.com", 80, open_timeout: 3) do |http|
    http.request_get(success_host)
  end
rescue Net::CircuitOpenError => e
  puts "   >> Semian is open: #{e.message}".brown
  puts "   !!! Semian open and no request made to shopify.com:80, even it is HEALTHY !!!".red.bold
end
puts
...
puts " >> 7. Request to HELTHY http://shopify.com:/index.html - success".cyan
Net::HTTP.start("shopify.com", 80, open_timeout: 3) do |http|
  http.request_get(success_host)
end
...
puts " >> 8. Request to HELTHY http://shopify.com:/index.html - success".cyan
Net::HTTP.start("shopify.com", 80, open_timeout: 3) do |http|
  http.request_get(success_host)
end
