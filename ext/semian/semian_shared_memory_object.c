...
create_and_resize_memory(key_t key, int byte_size, long permissions, int should_keep_req_byte_size_bool,
  void **data_copy, size_t *data_copy_byte_size, int *prev_mem_attach_count, semian_shm_object *ptr, VALUE self) {
...
semian_shm_object_acquire_memory(VALUE self, VALUE should_keep_req_byte_size)
{
  ...
  int shmid = create_and_resize_memory(key, ptr->byte_size, ptr->permissions, should_keep_req_byte_size_bool,
                                        &data_copy, &data_copy_byte_size, &prev_mem_attach_count, ptr, self);
  if (shmid < 0) {// failed
    if (data_copy)
      free(data_copy);
...
VALUE
semian_shm_object_delete_memory_inner (VALUE self)
{
...
semian_shm_object_delete_memory (VALUE self)
{
  ...
  if (!(WITHOUT_GVL(semian_shm_object_lock_without_gvl, (void *)ptr, RUBY_UBF_IO, NULL)))
    return Qnil;
  return rb_ensure(semian_shm_object_delete_memory_inner, self, semian_shm_object_synchronize_restore_lock_status, (VALUE)&status);
}
