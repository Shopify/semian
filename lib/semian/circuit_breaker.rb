module Semian
  class CircuitBreaker #:nodoc:
    ...
    def initialize(name, exceptions:, success_threshold:, error_threshold:,
                         error_timeout:, implementation:, half_open_resource_timeout: nil, error_window_timeout: nil)
      @name = name.to_sym
      @success_count_threshold = success_threshold
      @error_count_threshold = error_threshold
      @error_window_timeout = error_window_timeout ? error_window_timeout : error_timeout
    ...
    def push_time(window, time: Time.now)
      window.reject! { |err_time| err_time + @error_window_timeout < time.to_i }
      window << time.to_i
    end
