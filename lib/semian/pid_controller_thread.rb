...
module Semian
  class PIDControllerThread
    ...
    def start
      Semian.logger.info("Thread start called #{Process.pid} from #{caller}")
      @stopped = false

      update_proc = proc do
        loop do
          Semian.logger.info("PIDControllerThread Step1 #{Process.pid}")
          break if @stopped

          Semian.logger.info("PIDControllerThread Step2 #{Process.pid}")

          wait_for_window
          Semian.logger.info("PIDControllerThread Step3 #{Process.pid}")
          ...
          Semian.logger.info("PIDControllerThread Step4 #{Process.pid}")
        rescue => e
          Semian.logger.info("PIDControllerThread Step5 #{Process.pid}")
          Semian.logger&.warn("PID controller update thread error: #{e.message}")
          Semian.logger.info("PIDControllerThread Step6 #{Process.pid}")
        end
        Semian.logger.info("PIDControllerThread Step7 #{Process.pid}")
      end
    ...
    def stop
      Semian.logger.info("Thread stop called #{Process.pid} from #{caller}")
      @stopped = true
      @update_thread&.kill
      @update_thread = nil
    end
    ...
    def register_resource(circuit_breaker)
      ...
      Semian.logger.info("Thread register called #{Process.pid} with resource #{circuit_breaker.name} from #{caller}")
    ...
    def unregister_resource(circuit_breaker)
      ...
      Semian.logger.info("Thread unregister called #{Process.pid} with resource #{circuit_breaker.name} from #{caller}")
