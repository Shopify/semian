...
module ActiveRecord
  module ConnectionAdapters
    class ActiveRecordPostgreSQLAdapterTest < Minitest::Test
      ...
      def setup
        super
        @proxy = Toxiproxy[:semian_test_postgres]
        Semian.destroy(:postgres_testing)
        ...
        @configuration = {
          ...
          port: SemianConfig["postgres_toxiproxy_port"],
          read_timeout: 2,
          write_timeout: 2,
      ...
      def test_semian_identifier
        assert_equal(:postgres_testing, @adapter.semian_identifier)

        adapter = trilogy_adapter(host: "127.0.0.1", semian: { name: nil })

        assert_equal(:"postgres_127.0.0.1:13306", adapter.semian_identifier)

        adapter = trilogy_adapter(host: "shopify.com", port: 42, semian: { name: nil })

        assert_equal(:"postgres_shopify.com:42", adapter.semian_identifier)
      end
      ...
      def test_adapter_does_not_modify_config
        ...
        PostgreSQLAdapter.new(@configuration)
      ...
      def test_connection_errors_open_the_circuit
        @proxy.downstream(:latency, latency: 2200).apply do
          ...
          assert_raises(PostgreSQLAdapter::CircuitOpenError) do
            @adapter.execute("SELECT 1;")
          end
        end
      end
      ...
      def test_query_errors_do_not_open_the_circuit
        ...
        refute_kind_of(PostgreSQLAdapter::CircuitOpenError, err)
      end
      ...
      def test_read_timeout_error_opens_the_circuit
        ...
        # After PostgreSQLAdapter::CircuitOpenError check regular queries are working fine.
      ...
      def test_connect_instrumentation
        ...
        subscriber = Semian.subscribe do |event, resource, scope, adapter|
          ...
          assert_equal(Semian[:postgres_testing], resource)
      ...
      def test_other_postgres_errors_are_not_tagged_with_the_resource_identifier
      ...
      def test_resource_acquisition_for_connect
        ...
        Semian[:postgres_testing].acquire do
          error = assert_raises(PostgreSQLAdapter::ResourceBusyError) do
            trilogy_adapter.send(:connect) # Avoid going through connect!, which will call #active?
          end

          assert_equal(:postgres_testing, error.semian_identifier)
        end
      end
      ...
      def test_resource_acquisition_for_query
        ...
        Semian[:postgres_testing].acquire do
          assert_raises(PostgreSQLAdapter::ResourceBusyError) do
            @adapter.execute("SELECT 1;")
          end
        end
      end
      ...
      def test_resource_timeout_on_connect
        @proxy.downstream(:latency, latency: 500).apply do
          background do
            assert_raises(PostgreSQLAdapter::CircuitOpenError) { @adapter.connect! }
          end
      ...
      def test_query_allowlisted_returns_false_for_binary_sql
        ...
        refute(Semian::ActiveRecordPostgreSQLAdapter.query_allowlisted?(binary_query))
      end
      ...
      def test_circuit_open_errors_do_not_trigger_the_circuit_breaker
        @proxy.down do
          ...
          error = Semian[:postgres_testing].circuit_breaker.last_error
      ...
      def postgresql_adapter(**config_overrides)
        PostgreSQLAdapter.new(@configuration.merge(config_overrides))
      end
    end
  end
end
