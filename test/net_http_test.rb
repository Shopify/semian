...
class TestNetHTTP < Minitest::Test
  ...
  DEFAULT_SEMIAN_CONFIGURATION = proc do |host, port|
    next nil if host == SemianConfig['toxiproxy_host'] && port == SemianConfig['toxiproxy_port'] # disable if toxiproxy
  ...
  def test_semian_identifier
    with_server do
      with_semian_configuration do
        Net::HTTP.start(SemianConfig['toxiproxy_host'], SemianConfig['server_toxic_port']) do |http|
          assert_equal "nethttp_#{SemianConfig['toxiproxy_host']}_#{SemianConfig['server_toxic_port']}", http.semian_identifier
        end
      end
    end
  end
  ...
  def test_trigger_open
    with_semian_configuration do
      with_server do
        ...
        uri = URI("http://#{SemianConfig['toxiproxy_host']}:#{SemianConfig['server_toxic_port']}/200")
  ...
  def test_bulkheads_tickets_are_working
    ...
    with_semian_configuration(options) do
      with_server do
        http_1 = Net::HTTP.new(SemianConfig['toxiproxy_host'], SemianConfig['server_toxic_port'])
        http_1.semian_resource.acquire do
          http_2 = Net::HTTP.new(SemianConfig['toxiproxy_host'], SemianConfig['server_toxic_port'])
          http_2.semian_resource.acquire do
            assert_raises Net::ResourceBusyError do
              Net::HTTP.get(URI("http://#{SemianConfig['toxiproxy_host']}:#{SemianConfig['server_toxic_port']}/"))
            end
          end
        end
      end
  ...
  def test_get_is_protected
    with_semian_configuration do
      with_server do
        open_circuit!
        assert_raises Net::CircuitOpenError do
          Net::HTTP.get(URI("http://#{SemianConfig['toxiproxy_host']}:#{SemianConfig['server_toxic_port']}/200"))
        end
      end
    end
  end
  ...
  def test_instance_get_is_protected
    with_semian_configuration do
      with_server do
        ...
        assert_raises Net::CircuitOpenError do
          http = Net::HTTP.new(SemianConfig['toxiproxy_host'], SemianConfig['server_toxic_port'])
          http.get("/")
        end
      end
    end
  ...
  def test_custom_raw_semian_options_work_with_lookup
    with_server do
      ...
      semian_config["development"]["nethttp_#{SemianConfig['toxiproxy_host']}_#{SemianConfig['server_toxic_port']}"] = DEFAULT_SEMIAN_OPTIONS
  ...
  def test_custom_raw_semian_options_can_disable_with_invalid_key
    with_server do
      ...
      with_semian_configuration(semian_configuration_proc) do
        ...
        http = Net::HTTP.new(SemianConfig['toxiproxy_host'], SemianConfig['server_toxic_port'] + 100)
        assert_equal true, http.disabled?
      end
    end
  end
  ...
  def test_multiple_different_endpoints_and_ports_are_tracked_differently
    with_semian_configuration do
      ...
      ports.each do |port|
        reset_semian_resource(hostname: SemianConfig['toxiproxy_host'], port: port.to_i)
      end
    ...
    with_semian_configuration do
      with_server(ports: [SemianConfig['server_port'] + 100]) do |_, port|
        with_toxic(hostname: SemianConfig['server_host'], upstream_port: port, toxic_port: port + 1) do |name|
          open_circuit!(hostname: SemianConfig['toxiproxy_host'], toxic_port: port + 1, toxic_name: name)
        end
      end
      with_server(ports: [SemianConfig['server_port'] + 100], reset_semian_state: false) do |_, port|
        with_toxic(hostname: SemianConfig['server_host'], upstream_port: port, toxic_port: port + 1) do |_|
          assert_raises Net::CircuitOpenError do
            Net::HTTP.get(URI("http://#{SemianConfig['toxiproxy_host']}:#{port + 1}/200"))
          end
        end
      end
    end
  ...
  def open_circuit!(hostname: SemianConfig['toxiproxy_host'], toxic_port: SemianConfig['server_toxic_port'], toxic_name: "semian_test_net_http")
  ...
  def close_circuit!(hostname: SemianConfig['toxiproxy_host'], toxic_port: SemianConfig['server_toxic_port'])
  ...
  def with_server(ports: [SemianConfig['server_port']], reset_semian_state: true)
    ports.each do |port|
      ...
      begin
        ...
        reset_semian_resource(hostname: SemianConfig['toxiproxy_host'], port: port) if reset_semian_state
    ...
    Toxiproxy.populate([
      {
        ...
        listen: "#{SemianConfig['toxiproxy_host']}:#{toxic_port}",
      },
    ])
